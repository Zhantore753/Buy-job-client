...
const SupportCreate = () => {
    ...
    const createHandler = () => {
        ...
        if(title.length < 4 || title.length > 51){
            setValid([true, 'Заголовок должен быть не меньше 4 и не больше 50 символов']);
            offValidByTime();
            return;
        }
        if(description.length < 14 || description.length > 1000){
            setValid([true, 'Описание должно быть не меньше 14 и не больше 1000 символов']);
            offValidByTime();
            return;
        }
        dispatch(createTicket(title, description, Date.now(), files))
        .then(async res =>{
            await setValid(res);
        });
        offValidByTime();
            ...
            <div className="container">
                ...
                <div className="placing-order__inner">
                    ...
                    <form className="placing-order__desc-col placing-ticket">
                        ...
                        <ul className="placing-order__addedfiles placing-ticket__addedfiles">
                            Ничего не было добавленно
                        </ul>
                    </form>
                    <div className="placing-order__desc-btn placing-ticket__btn">
                        <button disabled={disabledBtn} onClick={createHandler}>Отправить вопрос</button>
                    </div>
                    {valid[0] === 200 ?
